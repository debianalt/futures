...
## 6. Identification and Screening Flow (revised, Aug 2026)
...
narratively for the framework; they are not part of the empirical corpus and
are not counted above.

Every count in this flow is generated by `corpus_report.py` from
`db/coding.csv` and `db/additional_records.csv`; the manuscript, the
supplementary information and this protocol must match that report exactly.
